...
MIN_THREE_YEAR_OBSERVATIONS = 720
THREE_YEAR_COVERAGE_TOLERANCE_DAYS = 31
...
def build_valuation_factors(
    ...
    trade_date: str,
    *,
    require_full_3y_history: bool = False,
) -> pd.DataFrame:
        ...
        rows.append(
            {
                ...
                "valuation_pe_percentile_3y": _valuation_percentile(
                    stock_history,
                    "pe_ttm",
                    row["pe_ttm"],
                    trade_date,
                    require_full_3y_history=require_full_3y_history,
                ),
                "valuation_pb_percentile_3y": _valuation_percentile(
                    stock_history,
                    "pb",
                    row["pb"],
                    trade_date,
...
def _valuation_percentile(
    ...
    *,
    ...
    if require_full_3y_history:
        minimum_date = date.fromisoformat(trade_date) - timedelta(
            days=365 * 3 - THREE_YEAR_COVERAGE_TOLERANCE_DAYS
        )
        history_dates = (
            pd.to_datetime(history["trade_date"], errors="coerce").dropna()
            if "trade_date" in history.columns
            else pd.Series(dtype="datetime64[ns]")
        )
        if (
            len(values) < MIN_THREE_YEAR_OBSERVATIONS
            or history_dates.empty
            or history_dates.min().date() > minimum_date
        ):
            return pd.NA
